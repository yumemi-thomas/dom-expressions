...
visits. One
known shape still fails reconciliation deliberately — a `children` attribute
on a nested non-void element, where this fork emits nothing and the parity
target inserts — because that failure is the divergence's only detection
signal. `transform()` output is unchanged.
